...
class PreProcess():
    ...
    def __init__(self,Path_to_storage,Path_to_CT_scans,Path_to_Labels,seed=123,Label_values=[1],Resize=False,
                 Patient_nr_index=1,axis="Axial",Manual_patients=None):
    ...
    def Extract_positive_patches(self,Train_image_dir,Train_label_dir):
        ...
        def load_data (Path_to_CT, Path_to_annoation):
            ...
            orginal_data[orginal_data>self.Max_hounsfield]=self.Max_hounsfield

            if self.axis=="Sagital":
                axis=1
            ...
            if self.axis == "Sagital" or self.axis == "Coronal":
                ...
                if (Pixel_dim_im[1:4]==[Pixel_dim_im[1],Pixel_dim_im[1],Pixel_dim_im[1]]).all() and (Pixel_dim_lab[1:4]==[Pixel_dim_lab[1],Pixel_dim_lab[1],Pixel_dim_lab[1]]).all():
                    ...
                    #Rescaled_scan,rescaled_header = Rescale_scan(orginal_data, Header_data_im)
                    #Rescaled_label,rescaled_labelheader = Rescale_scan(label_data, Header_data_lab,label=True)

                Rescaled_scan = Functions.Normalise_Nifti_file([Rescaled_scan], Mute=True)[0]
                Rescaled_scan, Rescaled_label,x = Swap_axis(Rescaled_scan, Rescaled_label, axis=axis)
                orginal_data=Rescaled_scan
                label_data=np.round(Rescaled_label)

            else:
                orginal_data = Functions.Normalise_Nifti_file([orginal_data],Mute=True)[0]
                Rescaled_scan, Rescaled_label,x = Swap_axis(orginal_data, label_data, axis=0,Swap=False)
                orginal_data=Rescaled_scan
                label_data=np.round(Rescaled_label)

            if self.Resize:
                if orginal_data.shape[0]!=self.Resize[0] and orginal_data.shape[1]!=self.Resize[1]:
                    Increase_fraction=orginal_data.shape[0]/self.Resize[0]
                    self.min_nr_pixels=int(self.Min_lesion_size/(pixel_dim[1]*Increase_fraction))
                    orginal_data=Functions.Resize_Patient_data(orginal_data,self.Resize,Mute=True)
                    label_data=Functions.Resize_Patient_data(label_data,self.Resize,Mute=True)
...
axis="Sagital"
Experiment_name="Sagital_BSv2_v2"
Path_to_yolo_folder="/home/mleeuwen/Deep learning Models/Total_bone_detector/yolov5"
Path_to_input_images="/home/mleeuwen/DATA/Reshaped_dataset_TSv2/Images"
Path_to_input_labels="/home/mleeuwen/DATA/Reshaped_dataset_TSv2/Labels"
...
Path_to_reference_dataset="/home/mleeuwen/Deep learning Models/Total_bone_detector/datasets/Axial_BSv2"
...
Class=PreProcess(Path_to_storage_directory,Path_to_input_images,Path_to_input_labels,
                 Label_values=Label_values,Resize=False,Patient_nr_index=Patient_nr_index,axis=axis,Manual_patients=Desired_patients)
