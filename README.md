...
```
- ```Affected_Bones.PNG ``` : This is a visualization of the bones in which the bone abnormalities are located. An example of such an image is shown on the top page of this page. This file is generated by using the ```utils/Atlas.nii``` file, which is a bone mask created by using bone labels from the TotalSegmentator dataset.
