...
class Data_processing():
    ...
    def Save_image_data_as_nifti(self,save_path,image_names,Data,Header,Auto_name_gen=False,Segmentation=False,Mute=False):
        """
        Desription:
    ...
    def Loading_Numpy_files(self,path_to_numpy_folder,File_names=[""],Load_all=False,Resize=False,Resolution=(768,768),Mute=False,Binarize_label=False):
        """
        Description:
            This function can load in the nifti files that are stored in a certain folder. This function is needed when you have
            previously loaded the dicom images and saved them in a .nii format. This function loads the data much faster than repeatedly,
        ...
        Mute: Boolean
            Determine whetehr you want to print statements during running of the code
        Binarize_label: Boolean
            Determine whether you want to binarize all of the. Default value is False
        Returns
        -------
        Nifti_files:TYPE
            The contents of the loaded nifti files.
        ...
        All_files_in_folder=os.listdir(path_to_numpy_folder)
        Files_in_folder=[string for string in All_files_in_folder if string.find(".npy")!=-1] #Remove non patient files in directory
        Numpy_files=[]
        Headers=[]
        if Load_all==True:
            Files_to_load=Files_in_folder
        ...
        if not Mute:        
            print("----Start Loading Files---")       
        for NF in tqdm(Files_to_load,disable=Mute):
            Load_path=os.path.join(path_to_numpy_folder,NF)
            im_data=np.load(Load_path)
            if Resize and (im_data.shape[0],im_data.shape[1])!=Resolution:
                im_data=self.Resize_Patient_data(im_data, Resolution,Mute=Mute)
            if Binarize_label:
                Thresholded_lab=np.zeros(im_data.shape)
                ...
                im_data=Thresholded_lab
                Thresholded_lab=[]
            Numpy_files.append(im_data)
            Headers.append(None)
        if not Mute:
            print("---Numpy Data loaded Succesfully!---")
        

        return  Numpy_files,Headers
